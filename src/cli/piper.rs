...
fn stream_klatt_stdin_to_frames(
    ...
    stream_rendered_stdin_to_frames(
        piper_args,
        frame_tx,
        "failed to read stdin for listenbury say -",
        "failed to send Klatt stdin audio to playback",
        |_, text| synthesize_klatt_for_say(text),
    )
}
...
fn stream_hifigan_stdin_to_frames(
    ...
    stream_rendered_stdin_to_frames(
        piper_args,
        frame_tx,
        "failed to read stdin for listenbury say --hifigan -",
        "failed to send HiFi-GAN stdin audio to playback",
        |args, _| synthesize_hifigan_for_say(args),
    )
}
...
fn stream_speecht5_stdin_to_frames(
    ...
    stream_rendered_stdin_to_frames(
        piper_args,
        frame_tx,
        "failed to read stdin for listenbury say --speecht5 -",
        "failed to send SpeechT5 stdin audio to playback",
        |args, _| synthesize_speecht5_for_say(args),
    )
}
...
fn stream_piper_stdin_to_frames(
    ...
    stream_rendered_stdin_to_frames(
        ...
        frame_tx,
        "failed to read stdin for listenbury say -",
        "failed to send Piper stdin audio to playback",
        move |_, text| {
            tts.enqueue(MouthSyntheticPlan::from(SyntheticUnit::FullTurn(
                text.to_string(),
            )))?;
...
fn stream_mbrola_stdin_to_frames(
    ...
    stream_rendered_stdin_to_frames(
        piper_args,
        frame_tx,
        "failed to read stdin for listenbury say --diphone -",
        "failed to send MBROLA stdin audio to playback",
        move |_, text| {
            tts.enqueue(MouthSyntheticPlan::from(SyntheticUnit::FullTurn(
                text.to_string(),
            )))?;
...
fn stream_rendered_stdin_to_frames(
    piper_args: SayArgs,
    frame_tx: crossbeam_channel::Sender<Vec<AudioFrame>>,
    read_context: &'static str,
    send_context: &'static str,
    mut render: impl FnMut(&SayArgs, &str) -> Result<Vec<AudioFrame>>,
) -> Result<()> {
    let stdin = std::io::stdin();
    for line in stdin.lock().lines() {
        let text = line.context(read_context)?;
        let text = text.trim();
        if text.is_empty() {
            continue;
        }
        let line_args = piper_args.clone_for_text(text);
        if line_args.dump_phonemes {
            print_say_phonemes(&line_args)?;
        }
        if line_args.dump_piper_tensors {
            print_say_piper_tensors(&line_args)?;
        }
        if line_args.dump_phone_plan {
            print_say_phone_plan(&line_args)?;
            continue;
        }
        let frames = render(&line_args, text)?;
        frame_tx.send(frames).context(send_context)?;
    }
    Ok(())
}
