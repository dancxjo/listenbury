...
cuda *args:
    CUDA_LIBRARY_PATH="{{cuda-library-path}}" RUSTFLAGS="{{cuda-rustflags}}" cargo run --features "asr-whisper-cuda llama-cpp-cuda" -- "$@"

# Start Pete's stream of consciousness through the CUDA feature path.
go *args:
    @just cuda go "$@"
...
build-cuda:
    CUDA_LIBRARY_PATH="{{cuda-library-path}}" RUSTFLAGS="{{cuda-rustflags}}" cargo build --features "asr-whisper-cuda llama-cpp-cuda"
...
check-cuda:
    CUDA_LIBRARY_PATH="{{cuda-library-path}}" RUSTFLAGS="{{cuda-rustflags}}" cargo check --no-default-features --features "asr-whisper-cuda llama-cpp-cuda piper-compat model-download"
