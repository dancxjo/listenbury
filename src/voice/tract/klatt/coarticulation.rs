...
pub(crate) fn apply_neighbor_influence(targets: &[PhoneRenderTarget]) -> Vec<PhoneRenderTarget> {
    ...
    for idx in 0..adjusted.len() {
        let phone = adjusted[idx].phone.ipa.clone();
        let f0_hz = adjusted[idx].f0_hz;
        let left_phone = idx
            .checked_sub(1)
            .map(|p| adjusted[p].phone.ipa.clone())
            .unwrap_or_default();
        let right_phone = adjusted
            .get(idx + 1)
            .map(|target| target.phone.ipa.clone())
            .unwrap_or_default();
        ...
        if is_vowel(phone.as_str()) {
            let left_bias = consonant_f2_bias(left_phone.as_str()).unwrap_or(0.0);
            let right_bias = consonant_f2_bias(right_phone.as_str()).unwrap_or(0.0);
            filter.f2_hz +=
...
fn consonant_f2_bias(phone: &str) -> Option<f32> {
    Some(match phone {
        "k" | "ɡ" | "ŋ" => -160.0,
        "p" | "b" | "m" | "w" => -80.0,
        "t" | "d" | "n" | "s" | "z" | "l" | "ɹ" => 110.0,
